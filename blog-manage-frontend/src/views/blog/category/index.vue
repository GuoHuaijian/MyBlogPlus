<template>
  <div>
      ...
      <el-dialog
        ...
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          <el-form-item label="类别">
            <el-input v-model="formInline.categoryTitle" placeholder="类别"></el-input>
          </el-form-item>
    ...
    <el-button type="primary" @click="editCategory(id)">修 改</el-button>
  </span>
      </el-dialog>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        tableColumn: [
          ...
          { field: 'categoryTitle', title: '类别',showOverflow: true},
    ...
    methods: {
      findList () {
        const that = this;
        CategoryList(this.tablePage.currentPage,this.tablePage.pageSize).then(function(response) {
      ...
      addCategory(){
        createCategory(this.formInline).then((res) =>{
          if (res.code = 20000){
            this.$message({
              type: 'success',
              message: '添加成功!'
      ...
      editCategory(id){
        updateCategory(this.formInline,id).then((res) =>{
          if (res.code = 20000){
            this.$message({
              type: 'success',
              message: '修改成功!'
